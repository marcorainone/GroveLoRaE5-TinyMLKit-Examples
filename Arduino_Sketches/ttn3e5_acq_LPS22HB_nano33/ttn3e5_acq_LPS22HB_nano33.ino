// **********************************************************************************
// ttn3e5_acq_LPS22HB_nano33.ino
//             v1 2023-10-25, ICTP Wireless Lab
// Programmer: Marco Rainone - ICTP Wireless Lab
// 
// Read LPS22HB sensor pressure and temperature and send values to TTN3 application.
//
// Note: for Arduino Nano 33 Sense and lite version
// https://forum.arduino.cc/t/a-difference-between-a-n-33-ble-sense-vs-sense-lite/1030305/4
// the BLE Sense Lite does not have onboard LPS22HB temperature and humidity sensor , 
// but rather has LPS22HB pressure sensor that can also be used to read temperature.
// This means that, it is technically possible to sense temperature with the board (but not humidity) as shown in the examples 26 of the LPS22HB sensor library.
//
...
#include <stdint.h>
#include <stdlib.h>
#include <Arduino_LPS22HB.h>			// LPS22HB, Pressure and temperature
...
// sensor values
float temperature; 										// LPS22HB Temperature
float pressure; 										// LPS22HB Pressure
...
void setup() {
    ...
    Serial.println("Send LPS22HB pressure and temperature to TTN application\r\n");
...
void loop() {
	...
	case JOIN_LORAWAN_NETWORK:
		...
		{
			state = READ_LPS22HB_SENSOR;		// after joining, start to acquire data
		}
		else
		{
			// network not joined
		...
		break;
	case READ_LPS22HB_SENSOR:					// read pressure and temperature
	...
	case SEND_LORAWAN_MSG:	
		...
		// prepare msg with pressure and temperature
		int iPress, iTemp;
		
		iPress = int(pressure * 100.0);
		iTemp  = int(temperature * 100.0);
		
		snprintf(msg, sizeof(msg), "AT+CMSGHEX=\"%04X%04X\"", iPress, iTemp);
	...
	case WAIT_BEFORE_NEW_SEND:
		...
		delay(waitMsec);
		// acquire new sensor temperature and humidity values
		state = READ_LPS22HB_SENSOR;
		break;
	default:
		state = INITIALIZE_LORAE5;
    }
