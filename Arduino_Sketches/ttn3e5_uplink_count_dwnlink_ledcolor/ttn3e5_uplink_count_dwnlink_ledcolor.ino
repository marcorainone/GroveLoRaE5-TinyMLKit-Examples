// **********************************************************************************
// ttn3e5_uplink_count_dwnlink_ledcolor.ino
//             v1 2023-10-25, ICTP Wireless Lab
// Programmer: Marco Rainone - ICTP Wireless Lab
// 
// Demostrative program to send an uplink message to a TTN3 application with these fields:
// 1. a 32 bit counter value
// 2. the color status of the RGB led
//
...
static unsigned int RGBcolor;							// color of RGB led, modified by a downlink message
...
// decode downlink message stored in msg as hexadecimal string
static void DecodeDownlinkMsg(void)
{
	unsigned char byte0;
	
	// The first 2 hexadecimal characters of the downlink message represent the byte containing the RGB LED color
	sscanf(msg, "%2hhx", &byte0);			// decode the representation of the first byte in msg
	byte0 = byte0 & 0x03;					// mask the 2 low order bits (4 values)
	switch(byte0)
	{
		case 0	:							// RGB led OFF
			RGBcolor = 0;
		...
		case 1	:							// RGB led RED
			RGBcolor = RED;
			Serial.println("RGB led color: RED");
			break;
		case 2	:							// RGB led BLUE
			RGBcolor = BLUE;
			Serial.println("RGB led color: BLUE");
			break;
		case 3	:							// RGB led GREEN
			RGBcolor = GREEN;
			Serial.println("RGB led color: GREEN");
			break;
	}
}
...
void setup() {
	...
	RGBcolor = 0;								// RGB led initially off
	state = INITIALIZE_LORAE5;					// initialize state
}

void loop() {
	setLed(LED_RGB, RGBcolor);					// update the RGB color in base of RGBcolor variable, modified by a Downlink message
	...
	case SEND_LORAWAN_MSG:
		...
		snprintf(msg, sizeof(msg), "AT+CMSGHEX=\"%08X%02X\"", counter, RGBcolor);		// form the uplink message
		if(cmdLoraE5(msg, "Done", 5000))
		{
			if(RxDownlinkMsg(rx_buff))
			{
				DecodeDownlinkMsg();			// decode downlink msg received from TTN and execute commands.
			}
		}
		else
		{
